...
float getPUPPIweight(float puppipt, float puppieta ){
  ...
  double barrleMean[8]={1.11751,1.10595,1.099,1.10518,1.10258,1.10678,1.1166,1.1274};
  double endcapMean[8]={1.29907,1.22699,1.20171,1.18945,1.1691,1.1636,1.14132,1.18613};
 double barrlex[8]={339.042,423.44,563.086,653.216,748.773,838.752,1155.91,1382.22};
 double endcapx[8]={308.105,382.523,489.189,553.159,624.125,662.874,853.149,937.529};
 TGraph* tg1=new TGraph(8,barrlex,barrleMean);
 TGraph* tg2=new TGraph(8,endcapx,endcapMean);
 ...
 dir->GetObject("treeMaker",tree);
  ...
  if((jEntry+1)%2)continue;
    ...
    for(int ig=0; ig < nGenPar; ig++){
      ...
      int gmpid = abs(genGMomParId[ig]);

      if( (pid==12 || pid==14 || pid==16) && genParSt[ig]==\
1
	  && ((mpid>=500 && mpid<600) ||
	      (gmpid>=500 && gmpid<600) ||
	      (mpid>=5000 && mpid<6000) ||
	      (gmpid>=5000 && gmpid<6000)
	      )
	...
	//     if(thisJet->DeltaR(*thisNeutrino)<0.8)
	//       {
	// 	hasNeutrinos=true;
	// 	break;
	//       }
...
void xAna_hh_massResolution(int pt_down=60,int pt_up=0){
  xAna_hh_massResolutionBase("/data7/syu/special_study/80X_miniAOD/80X_neutrino/BulkGravTohhTohbbhbb/B900.root","B900.root",pt_up,pt_down);
  xAna_hh_massResolutionBase("/data7/syu/special_study/80X_miniAOD/80X_neutrino/BulkGravTohhTohbbhbb/B1200.root","B1200.root",pt_up,pt_down);
  ...
  xAna_hh_massResolutionBase("/data7/syu/special_study/80X_miniAOD/80X_neutrino/BulkGravTohhTohbbhbb/B1600.root","B1600.root",pt_up,pt_down);
  xAna_hh_massResolutionBase("/data7/syu/special_study/80X_miniAOD/80X_neutrino/BulkGravTohhTohbbhbb/B1800.root","B1800.root",pt_up,pt_down);
  xAna_hh_massResolutionBase("/data7/syu/special_study/80X_miniAOD/80X_neutrino/BulkGravTohhTohbbhbb/B2500.root","B2500.root",pt_up,pt_down);
